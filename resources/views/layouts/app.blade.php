...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        @if(Auth::user()->isModuleEnabled('staff'))
        <div class="nav-group" id="navGroupRegistry">
            ...
            <div class="nav-group-children {{ in_array(request()->segment(1), ['staff', 'family', 'report', 'ir']) ? 'open' : '' }}">
                <div class="nav-group-children-inner">
                    ...
                    @if(Auth::user()->isModuleEnabled('family'))
                    ...
                    @endif
                    @if((Auth::user()->isAdmin() || Auth::user()->isCeo()) && Auth::user()->isModuleEnabled('ir'))
        ...
        @if(Auth::user()->isModuleEnabled('training'))
        ...
        @if(Auth::user()->isModuleEnabled('travel'))
        ...
        @if((Auth::user()->isAdmin() || Auth::user()->isCeo()) && Auth::user()->isModuleEnabled('requests'))
...
function renderNotifList(items) {
    ...
    list.innerHTML = items.map(n => {
        ...
        return `
            <div class="notif-item ${n.is_read ? '' : 'unread'}" onclick="notifMarkRead(${n.id}, '${n.link}', '${n.type}')">
