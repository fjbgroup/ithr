...
.md-stat-num{font-size:20px;font-weight:800;color:var(--text);line-height:1;font-family:'DM Sans',sans-serif}
...
.md-add-input{flex:1;border:1.5px solid var(--border);border-radius:8px;padding:8px 12px;font-size:13px;color:var(--text);font-family:'DM Sans',sans-serif;background:var(--surface);outline:none;transition:border-color .2s,box-shadow .2s;min-width:0}
.md-add-input:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(2,132,199,.1)}
.md-add-btn{display:inline-flex;align-items:center;gap:5px;border:none;border-radius:8px;padding:8px 16px;font-size:12px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;white-space:nowrap;color:#fff;background:var(--navy,#142b47)}
...
.md-edit-input{font-size:12px;font-weight:700;color:var(--text);background:var(--body-bg);border:1.5px solid var(--accent);border-radius:7px;padding:5px 10px;width:180px;font-family:'DM Sans',sans-serif;text-transform:uppercase;outline:none}
.md-btn-save{font-size:11px;font-weight:700;color:#fff;background:var(--navy,#142b47);border:none;border-radius:6px;padding:5px 11px;cursor:pointer;font-family:'DM Sans',sans-serif}
.md-btn-cancel{font-size:11px;font-weight:600;color:var(--muted);background:transparent;border:1.5px solid var(--border);border-radius:6px;padding:5px 9px;cursor:pointer;font-family:'DM Sans',sans-serif}
...
<div style="margin-bottom:24px">
  <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.07em;color:var(--muted);margin-bottom:5px">
    Masterdata &rsaquo; <span style="color:var(--accent)">Brands</span>
  </div>
  <h4 style="font-family:'DM Sans',sans-serif;font-weight:800;font-size:22px;color:var(--text);margin:0">Brands</h4>
  <p style="font-size:13px;color:var(--muted);margin:4px 0 0">Manage asset brands used in IT and Non-IT assets</p>
</div>

@php
  $total    = $brands->count();
  $used     = $brands->filter(fn($b) => ($b->it_count + $b->nit_count) > 0)->count();
  $itItems  = $brands->sum('it_count');
  $nitItems = $brands->sum('nit_count');
@endphp
...
<div class="md-card">
  ...
  <div class="md-card-head">
    <div class="md-card-title-row">
      <div class="md-card-accent" style="background:#0284c7"></div>
      <div class="md-card-meta">
        <h6>Asset Brands</h6>
        <small>Used in IT Assets and Non-IT Assets</small>
      </div>
      <span style="background:rgba(2,132,199,.1);color:#0284c7;border:1px solid rgba(2,132,199,.2);border-radius:20px;padding:4px 12px;font-size:11px;font-weight:700;display:inline-flex;align-items:center;gap:5px">
        <i class="bi bi-bookmark-fill" style="font-size:10px"></i> Masterdata
      </span>
    </div>
    <div class="md-stat-row">
      <div class="md-stat-box" style="background:rgba(2,132,199,.08);border-color:rgba(2,132,199,.2)">
        <div class="md-stat-num" style="color:#0284c7">{{ $total }}</div>
        <div class="md-stat-lbl">Total Brands</div>
      </div>
  ...
  <!-- Add row -->
  <form method="POST" action="{{ route('it.brands.store') }}" class="md-add-row">
    @csrf
    <input type="text" name="name" class="md-add-input" placeholder="New brand name, e.g. HP" required
  ...
  <div class="md-empty">
    ...
    <p>No brands yet</p>
    <span>Add a brand above to get started</span>
  </div>
  @else
  <div class="table-responsive">
    <table class="md-table">
      ...
      <tr>
        ...
        <td>
          <div id="label-{{ $brand->id }}" class="md-pill" style="background:rgba(2,132,199,.08);border-color:rgba(2,132,199,.25)">
          ...
          <form id="editform-{{ $brand->id }}" method="POST" action="{{ route('it.brands.update', $brand->id) }}" class="md-edit-form">
            @csrf
            <input type="text" name="name" value="{{ $brand->name }}" required class="md-edit-input" oninput="this.value=this.value.toUpperCase()">
            <button type="submit" class="md-btn-save">Save</button>
            <button type="button" class="md-btn-cancel" onclick="cancelEdit({{ $brand->id }})">Cancel</button>
          </form>
        </td>
        <td>
          <span class="md-count-pill" style="background:{{ $brand->it_count > 0 ? 'rgba(2,132,199,.08)' : 'var(--body-bg)' }};color:{{ $brand->it_count > 0 ? '#0284c7' : 'var(--muted)' }};border-color:{{ $brand->it_count > 0 ? 'rgba(2,132,199,.25)' : 'var(--border)' }}">
            {{ $brand->it_count }} item{{ $brand->it_count !== 1 ? 's' : '' }}
          </span>
        </td>
        <td>
          <span class="md-count-pill" style="background:{{ $brand->nit_count > 0 ? 'rgba(124,58,237,.08)' : 'var(--body-bg)' }};color:{{ $brand->nit_count > 0 ? '#7c3aed' : 'var(--muted)' }};border-color:{{ $brand->nit_count > 0 ? 'rgba(124,58,237,.25)' : 'var(--border)' }}">
            {{ $brand->nit_count }} item{{ $brand->nit_count !== 1 ? 's' : '' }}
          </span>
        </td>
        <td>
          @php $inUse = ($brand->it_count + $brand->nit_count) > 0; @endphp
          @if($inUse)
        ...
        <td>
          <div id="actions-{{ $brand->id }}" style="display:flex;gap:5px;justify-content:center">
            <button class="md-btn-sm btn-edit" onclick="startEdit({{ $brand->id }})" title="Rename">
