...
<div class="mb-4 flex flex-col md:flex-row md:items-end md:justify-between gap-3">
    <div>
        <h3 class="text-base font-extrabold text-[#3D2B1F] dark:text-slate-100 tracking-tight">View Faulty Report</h3>
    ...
    <a href="{{ route($routePrefix . '.damages.status', $isAdminRoute ? ['bucket' => 'drafts', 'mode' => $mode] : ['bucket' => 'drafts']) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[#8B5E3C] text-white text-[10px] font-black uppercase tracking-widest hover:bg-[#734C2F]">
...
<div class="px-2">
    <div class="damage-card bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-stone-200 dark:border-slate-700 overflow-hidden p-5 md:p-6">
        <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div>
                <p class="text-[10px] font-black uppercase tracking-widest text-[#8B5E3C]">Faulty Report Details</p>
                <h3 class="mt-2 text-sm font-extrabold text-[#3D2B1F] dark:text-slate-100">Faulty Report #{{ str_pad($record->maintenance_id, 4, '0', STR_PAD_LEFT) }}</h3>
        ...
        <div class="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4 text-[11px]">
            <div class="rounded-xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 px-4 py-3">
                ...
                <p class="mt-2 font-black text-[#3D2B1F] dark:text-slate-100">{{ strtoupper($record->reporter_name ?: '-') }}</p>
            ...
            <div class="rounded-xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 px-4 py-3">
                ...
                <p class="mt-2 font-black text-[#3D2B1F] dark:text-slate-100">{{ strtoupper($record->department_name ?: '-') }}</p>
            </div>
            <div class="rounded-xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 px-4 py-3">
                <p class="text-[9px] font-black uppercase tracking-widest text-stone-500">Ownership / Deployment</p>
                <p class="mt-2 font-black text-[#3D2B1F] dark:text-slate-100">Ownership: {{ strtoupper($record->ownership_type ?: '-') }}</p>
            ...
            <div class="rounded-xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 px-4 py-3">
                ...
                <p class="mt-2 font-black text-[#3D2B1F] dark:text-slate-100">Model: {{ strtoupper($record->model ?: '-') }}</p>
            ...
            <div class="rounded-xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 px-4 py-3">
                ...
                <p class="mt-2 font-black text-[#3D2B1F] dark:text-slate-100">Submitted: {{ $record->received_date ? \Carbon\Carbon::parse($record->received_date)->format('d M Y') : '-' }}</p>
            ...
            <div class="rounded-xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 px-4 py-3">
                ...
                <p class="mt-2 font-black text-[#3D2B1F] dark:text-slate-100">Source: {{ strtoupper(str_replace('_', ' ', $record->request_source ?: 'USER')) }}</p>
            ...
            <div class="md:col-span-2 rounded-xl border border-emerald-200 dark:border-emerald-900/70 bg-emerald-50 dark:bg-emerald-950/30 px-4 py-3">
                ...
                <p class="mt-2 font-black text-[#3D2B1F] dark:text-slate-100">Handover to ICT: {{ strtoupper($record->handover_person ?: '-') }} | {{ $record->handover_at ? \Carbon\Carbon::parse($record->handover_at)->format('d M Y, h:i A') : '-' }}</p>
            ...
            <div class="md:col-span-2 rounded-xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 px-4 py-3">
                ...
                <p class="mt-2 font-bold text-[#3D2B1F] dark:text-slate-100 leading-relaxed">{{ $record->problem_possible ?: ($record->issue_description ?: '-') }}</p>
            </div>
            <div class="md:col-span-2 rounded-xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 px-4 py-3">
                <p class="text-[9px] font-black uppercase tracking-widest text-stone-500">Remarks / Replacement Details</p>
                <p class="mt-2 font-bold text-[#3D2B1F] dark:text-slate-100 leading-relaxed">{{ $record->remarks ?: 'No additional remarks.' }}</p>
            ...
            <div class="md:col-span-2 rounded-xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 px-4 py-3">
                ...
                @else
                <p class="mt-2 font-bold text-[#3D2B1F] dark:text-slate-100">No evidence uploaded.</p>
                @endif
            </div>
        </div>
    </div>
