...
.nav-link.active,.nav-link.active-sidebar,.nav-link.sidebar-active{background:rgba(56,189,248,.18);color:var(--sky);font-weight:600}
...
.topbar{
  ...
  padding:0 28px;min-height:60px;display:flex;align-items:center;gap:16px;
...
/* ── CONTENT SURFACE (User pages use this container) ── */
        ...
        <div id="notificationDropdown" class="hidden" style="position:absolute;top:calc(100% + 10px);right:0;width:320px;max-width:86vw;background:#fff;border:1px solid var(--border);border-radius:12px;box-shadow:0 18px 42px rgba(15,23,42,.16);z-index:7000;overflow:hidden">
          ...
          <div style="max-height:360px;overflow-y:auto">
            ...
            <div style="border-bottom:1px solid #f8fafc;{{ is_null($notification->read_at) ? 'background:#f0f9ff' : 'background:#fff' }}">
              <div style="display:flex;align-items:start;justify-content:space-between;gap:12px;padding:12px 16px">
                <div style="min-width:0;flex:1">
                  <p style="font-size:10px;font-weight:{{ is_null($notification->read_at) ? '700' : '600' }};color:var(--text);text-transform:uppercase;letter-spacing:.06em">{{ $notification->data['title'] ?? 'Notification' }}</p>
                  <p style="margin-top:3px;font-size:11px;color:var(--muted);line-height:1.4">{{ $notification->data['message'] ?? '' }}</p>
                  <p style="margin-top:3px;font-size:10px;color:#94a3b8">{{ $notification->created_at?->diffForHumans() }}</p>
                </div>
                ...
                <form action="{{ route('wt.notifications.read', $notification->id) }}" method="POST">
                  ...
                  <button style="background:none;border:none;cursor:pointer;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--accent);white-space:nowrap;flex-shrink:0">Read</button>
                </form>
                @else
                <form action="{{ route('wt.notifications.read', $notification->id) }}" method="POST">
                  @csrf
                  <input type="hidden" name="redirect_url" value="{{ $notificationUrl }}">
                  <button style="background:none;border:none;cursor:pointer;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);white-space:nowrap;flex-shrink:0">Open</button>
                </form>
                @endif
              </div>
            </div>
